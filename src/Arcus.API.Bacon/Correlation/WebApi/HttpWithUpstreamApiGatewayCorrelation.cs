...
namespace Arcus.API.Bacon.Correlation.WebApi
{
    /// <summary>
    ...
    public class HttpWithUpstreamApiGatewayCorrelation : ICorrelationInfoAccessor
    {
        ...
        public HttpWithUpstreamApiGatewayCorrelation(
        ...
        {
            // TODO: Contribute Upstream :I added this to get the upstream operation id, but ideally this is configurable
            var upstreamOperationId = GetUpstreamOperationId(httpContext);
            if (String.IsNullOrWhiteSpace(upstreamOperationId) == false)
            {
                return upstreamOperationId;
            }

            var simpleOperationId = httpContext.TraceIdentifier;
        ...
        private string? GetUpstreamOperationId(HttpContext context)
        {
            ...
            if (context.Request.Headers.ContainsKey("Request-Id") && 
                context.Request.Headers.TryGetValue("Request-Id", out var requestId))
            {
                // Get the operation ID from the Request-Id (if you follow the HTTP Protocol for Correlation).
                var operationId = GetOperationId(requestId);

                // TODO: Contribute Upstream : Provide capability to interpret and pass parent id
                var parentOperationId = requestId;
                // Should be added as following: requestTelemetry.Context.Operation.ParentId = requestId;
                return $"{operationId}//{parentOperationId}";
            }
        ...
        {
            // TODO: Contribute Upstream : I added this to pass the upstream operation id as a response, but ideally this is configurable (opt-in)
            // Not sure if there's much value though
            var upstreamOperationHeader = GetUpstreamOperationId(httpContext);
            if (String.IsNullOrWhiteSpace(upstreamOperationHeader) == false)
            {
                httpContext.Response.OnStarting(() =>
                {
                    AddResponseHeader(httpContext, $"Upstream-{_options.Operation.HeaderName}", upstreamOperationHeader);
